...
- **Puffin content requires explicit sync**: The `puffin_content` column on games is not auto-populated. You must call `POST /api/admin/puffin/content?all=true` (backfill) or `?bggIds=...` (specific) to pull content from Puffin into the database. The frontend displays nothing until this sync runs. Key files: `src/lib/bgg/content-importer.ts`, `src/app/api/admin/puffin/content/route.ts`.
